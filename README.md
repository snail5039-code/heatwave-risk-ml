...
- 2022~2025년 전체 연령·65세 이상 학습 데이터 구축 완료
- 대상별 Random Forest 모델 학습·평가 및 저장 완료
- 위험 등급·외출 시간 안내 로직 구현 완료
- Streamlit 분석 앱과 Next.js 반응형 웹사이트 구현·배포 완료
- 전체 연령과 65세 이상 분석을 합친 **최종 통합 분석 보고서 작성 완료**

최종 산출물은 [통합 분석 보고서 PDF](docs/분석보고서/00_통합_분석보고서.pdf)와 [HWPX 원본](docs/분석보고서/00_통합_분석보고서.hwpx)에서 확인할 수 있습니다.
...
- 전체 연령 모델은 무작위 테스트에서 R² 0.846, 65세 이상 모델은 R² 0.837을 기록했습니다.
- 전체 연령 데이터에서는 최고기온과 환자 수의 상관계수가 약 0.722로 가장 높았고, 평균기온은 약 0.672, 최저기온은 약 0.548이었습니다.
- 전체 연령 모델의 변수 중요도에서 기온 관련 변수 3개의 비중이 약 80.7%로 나타나, 온열질환자 수 예측에서 기온이 핵심 설명 변수임을 확인했습니다.
- 과거 연도로 학습하고 2025년을 예측한 전체 연령 모델은 R² 0.686으로 무작위 분할보다 성능이 낮아졌습니다. 따라서 새로운 연도와 환자 수 급증 구간에서는 오차가 커질 수 있습니다.

## 해석 시 주의사항

- 두 모델은 **전국 단위 일일 신고 환자 수**를 예측하며 지역별 환자 수나 개인의 발병 확률을 예측하지 않습니다.
- 위험 등급과 외출 안내는 프로젝트 내부 기준의 참고 정보이며, 기상청 공식 폭염특보·의학적 진단·외출 안전 판정을 대신하지 않습니다.
- 모델은 신고 자료와 기상 자료의 범위에 영향을 받으며, 극단적인 환자 수 급증 구간을 낮게 예측할 수 있습니다.
- 고령자 공개 원본과 공식 연보의 2022~2024년 합계 차이는 원인이 확인되지 않아 임의 보정하지 않았습니다.
...
docs/
  ...
  기획/                     초기 기획 문서
  분석보고서/               최종 통합 보고서 원본·PDF와 대상별 분석 그래프
...
- [최종 통합 분석 보고서 PDF](docs/분석보고서/00_통합_분석보고서.pdf)
- [최종 통합 분석 보고서 HWPX](docs/분석보고서/00_통합_분석보고서.hwpx)
- [전체 연령·65세 이상 분석 그래프 안내](docs/분석보고서/README.md)

최종 보고서는 프로젝트 배경과 해결하려는 문제, 데이터 전처리, 탐색적 데이터 분석, 대상별 모델 성능, 2025년 연도 분리 평가, 서비스 구현 결과, 활용 범위와 공통 한계를 포함합니다.
